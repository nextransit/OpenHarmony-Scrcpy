            ...
            try:
                ...
                if self.decoder and self.decoder.is_ready():
                    ...
                    if rgb_array is not None:
                        ...
                        # 新增:若 decoder 标记连续坏帧,触发自动重连（HEVC 编码器故障兜底）
                        bad_count = getattr(self.decoder, "bad_frame_count", 0) if self.decoder else 0
                        if bad_count >= 30 and self.is_connected:
                            print_log(LogLevel.ERROR, self.log_title,
                                f"检测到连续 {bad_count} 帧 HEVC 异常 (edge_y/edge_x={getattr(self.decoder, "last_bad_frame_ratio", 0):.2f})，自动重连 server")
                            # 清空当前 frame_queue 后断开,触发上层 reconnect
                            while not self.frame_queue.empty():
                                try: self.frame_queue.get_nowait()
                                except queue.Empty: break
                            # 把 on_state_changed 链路上的连接器用 disconnect 触发
                            self.disconnect()
                            break
                        ...
                        try:
                            ...
                            if self.on_frame_decoded:
                                try:
                                    # 注意:不要在这里直接 canvas.after,
                                    # 否则解码线程与 Tk 主循环互相挤占,
                                    # 导致 FPS 反而下降。on_frame_decoded
                                    # 内部会用 _schedule_render_if_needed。
